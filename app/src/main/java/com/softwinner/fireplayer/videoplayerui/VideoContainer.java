...
public class VideoContainer extends FrameLayout {
    ...
    public void init(Context context, GestureDetector detector, VideoSurfaceTextureView videoView, MovieControlPanel controlpanel, ControlGestureListener controlGestureListener) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (this.mVideoView.isFullScreen() && event.getPointerCount() >= 2 && !this.handled && float_window_enable && (this.mVideoView.mWindowMode != 1 || !this.mControlPanel.getUiLockState())) {
    ...
    private void handleMutiTouch(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case 2:
                if (Math.sqrt(Math.pow((double) (x0 - x1), 2.0d) + Math.pow((double) (y0 - y1), 2.0d)) / this.lastDistance < 0.9d && !PresentationScreenMonitor.getInstance(this.mContext.getApplicationContext()).isWIFIDisplayOn()) {
                    ...
                    Intent intent = new Intent(this.mContext, FloatVideoService.class).setAction(FloatVideoService.ACTION_SWITCH_MODE);
